...
Component pages include preview, usage, actual source, install command, composition links, shared Crafter styling, dark mode, reset, and adjacent navigation. Classification describes responsibility, not DOM-node count. Templates and pages are described as future compositions, not empty catalog categories.
...
Verification: all 13 component routes prerender in the production build. Existing 34 tests pass, as do TypeScript and Biome. Browser checks covered mobile sidebar opening/search/navigation/closing, actual source display, style generator changes, dark dialog portal styling, Escape dismissal, and ZIP download from `/create`. Code panes scroll internally at 390px without widening the page. Desktop and mobile component-page axe checks show 0 violations, 37 passing rules, and 0 incomplete rules.

The shadcn group identifies primitive source, not an alternate visual theme. Site tokens and all documentation previews now use `themeVariables(defaultConfig)`. Style customization lives in `/create`, with a live preview of buttons, badges, fields and cards.
